...
#define DWR_VERSION "UNOFFICIAL - JUEF v3.0.3.19"
#define VERSION DWR_VERSION

#define DEFAULT_FLAGS "IVIAAVCEKACBAAAAAAAAAEAUAAQBAAAAABYCAAAA"
...
#define RANDOMIZE_FLUTE_MUSIC(x)  ((x->flags[16] & 0x20) >> 5) // 1
#define DONT_MUTE_JINGLES(x)      ((x->flags[16] & 0x10) >> 4) // 1
...
#define RETURN_ESCAPES(x)         ((x->flags[19] & 0xc0) >> 6) // 2
#define WARP_WHISTLE(x)           ((x->flags[19] & 0x30) >> 4) // 2
#define LEVELUP_REFILL(x)         ((x->flags[19] & 0x0c) >> 2) // 2
#define MAX_KEYS(x)               ((x->flags[19] & 0x03)     ) // 2
...
#define HINTS(x)                  ((x->flags[22] & 0x18) >> 3) // 2: vanilla, corrected, useful, one of the above
#define NORMALIZED_XP_GOLD(x)     ((x->flags[22] & 0x06) >> 1) // 2
#define WINTER_THEME(x)           ((x->flags[22] & 0x01)     ) // 1
